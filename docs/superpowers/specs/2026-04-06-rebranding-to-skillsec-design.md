...
This document outlines the full technical and visual rebranding of the project from **SkillAuditAI** to **SkillSec**.

## 1. Core Identity Changes

| Entity | Old Value | New Value |
| :--- | :--- | :--- |
| Project Name | SkillAuditAI | SkillSec |
| Domain | N/A | skillsec.dev |
| Go Module | `github.com/albertowar/skillauditai` | `github.com/albertowar/skillsec` |
| Binary Name | `skillaudit` | `skillsec` |
| Repository URL | `https://github.com/albertowar/skillauditai` | `https://github.com/albertowar/skillsec` |
| Primary Env Var | `SKILLAUDIT_API_KEY` | `SKILLSEC_API_KEY` |

## 2. Technical Implementation Plan

### 2.1 Go Module & Imports
- Update `go.mod`: Change `module github.com/albertowar/skillauditai` to `module github.com/albertowar/skillsec`.
- Mass update all `.go` files: Replace all internal import paths from `github.com/albertowar/skillauditai/...` to `github.com/albertowar/skillsec/...`.

### 2.2 Filesystem Changes
- Rename `cmd/skillaudit` directory to `cmd/skillsec`.
...
- Update `.github/workflows/release.yml`:
    - Change binary output names from `skillaudit-*` to `skillsec-*`.
...
- **Documentation Verification**: Preview the `mkdocs` site (if possible) to ensure no "SkillAuditAI" remains in titles or links.
